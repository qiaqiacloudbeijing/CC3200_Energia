...
char* MsgHandler::topicAckuseridEncode(char *epindex, char *gwindex) {
	static char temp[50] = {"\0"};
	memset(temp, 0, 50);
	sprintf(temp, "s/1/0/0/%s/%s/+/0/0/ackuserid", epindex, gwindex);
	return temp;
}

char* MsgHandler::topicRegisterEncode(char *epindex, char *gwindex) {
	static char temp[50] = {"\0"};
	memset(temp, 0, 50);
	sprintf(temp, "c/1/0/0/%s/%s/0/0/0/register", epindex, gwindex);
	return temp;
}

char* MsgHandler::topicReadEncode(char *gwindex, char *userid) {
	static char temp[80] = {"\0"};
	memset(temp, 0, 80);
	sprintf(temp, "s/1/+/0/+/%s/+/%s/+/read", gwindex, userid);
	return temp;
}

char* MsgHandler::topicWriteEncode(char *gwindex, char *userid) {
	static char temp[80] = {"\0"};
	memset(temp, 0, 80);
	sprintf(temp, "s/1/+/0/+/%s/+/%s/+/write", gwindex, userid);
	return temp;
}
...
char* MsgHandler::gwAckReadEncode(char *topic_srcuserid, char *devindex_read, char *offsetindex_read,
	...
	static char temp[120] = {"\0"};
	memset(temp, 0, 120);
	sprintf(temp, "%s/%s/%s/%s/%s/%d/", topic_srcuserid, devindex_read, offsetindex_read, 
		topic_offsetindex, epindex_read, num);
	return temp;
}
